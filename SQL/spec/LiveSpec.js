...
describe("Persistent Node Chat Server", function() {
  ...
  it("Should insert posted messages to the DB", function(done) {
    ...
    request({method: "POST",
            ...
            console.log('entering query successfully from livespec'),
            function () {
              /* Now if we look in the database, we should find the
               * posted message there. */
              var queryString = "SELECT * FROM messages WHERE content = ?";
  ...
  it("Should output all messages from the DB", function(done) {
    ...
    var queryString = "INSERT INTO messages (content, roomnames) values (? , ?)";
      ...
      function(err) {
        ...
        request("http://127.0.0.1:3000/classes/messages",
          function(error, response, body) {
            console.log('inside callback for second livespec test');
            var messageLog = JSON.parse(body);
            expect(messageLog[0].content).to.equal("Men like you can never change!");
            expect(messageLog[0].roomnames).to.equal("main");
            done();
          });
      });
  });
